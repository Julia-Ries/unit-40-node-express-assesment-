...
router.get('/', authUser, requireLogin, async function(req, res, next) {
  try {
    //BUG #2 FIX
    let users = await User.basicInfo();
...
router.patch('/:username', authUser, requireLogin, requireAdmin, async function(
  ...
  try {
    ...
    const invalidField = new ExpressError("Cannot update this field", 404)
    return next(invalidField);
  }
}); // end
...
router.delete('/:username', authUser, requireAdmin, async function(
  ...
  try {
    ...
    //BUG #5 FIX
    const invalidError = new ExpressError("User cannot be found", 404)
