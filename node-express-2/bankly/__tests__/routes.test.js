...
describe("POST /auth/login", function() {
  ...
  //BUG #1 Test
  test("should not allow incorrect username/password to login", async function() {
    const response = await request(app)
    .post("/auth/login")
    .send({
      username: "incorrect",
...
describe("DELETE /users/[username]", function() {
  ...
  //BUG 5 TEST
  test("should  404 error if user not found", async function() {
    const response = await request(app).delete("/users/unliu23");
    expect(response.statusCode).toBe(404);
  });
});
